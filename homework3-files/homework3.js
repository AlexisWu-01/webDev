...
function makeTable(id,headers,rows){
    ...
    tb.style.outline = '1px solid black'
    ...
    for (var i=0;i<rows.length;i++){
        ...
        for (var k=0;k<headers.length;k++){
            ...
            td.appendChild(document.createTextNode(rows[i][k]))
            row.appendChild(td)
        }
        row.style.borderTop = '1px solid black'
        row.style.borderCollapse='collapse'
        row.style.outlineColor ='red'
        tb.appendChild(row)
        
    }
    tb.style.borderCollapse = 'collapse'
    let dp = document.getElementById('display-area')
    dp.appendChild(tb)
    return tb
}


function sortTable(table,col,dir='asc'){
    var tb = table
    const c=col
    var sorting=true
    var sortFlag
    ...
    while (sorting){
        ...
        for (i=0;i<rows.length-1;i++){
            ...
            if ((x.innerHTML.toLowerCase()>y.innerHTML.toLowerCase())&&(dir=='dsc')){
                sortFlag=true
                break;
            }
...
function makeSortableTable(id,headers,rows){
    let tb = makeTable(id,headers,rows)
    let th = document.querySelectorAll('table th')
    for (var i=0;i<th.length;i++){
        th[i].setAttribute('class','sortable')
...
function makeFilterableList(filterId, listId, items){
    ...
    input.value = ''
    var candi_list = document.createElement('div')
    candi_list.setAttribute('id',listId)
    let dp = document.getElementById('display-area')
    dp.appendChild(label)
    label.appendChild(input)
    dp.appendChild(updateList(listId,items))

    let inputlist = []
    input.addEventListener('keyup',function(input){
        inputlist = input.target.value.split('')
           
        let contain = matchList(inputlist,items)
        dp.appendChild(updateList(listId,contain))
        })
    }
    


function matchList(inp_list,items){
    const input = inp_list.join('')
    let contain = []
    for (var i=0;i<items.length;i++){
        if(items[i].toLowerCase().includes(input.toLowerCase())){
            contain.push(items[i])
        }
    }
    return contain
}
...
function updateList(listId,items){
    let list = clearDisplay(listId)
    list.setAttribute('div',listId)
    for (var i=0;i<items.length;i++){
...
function setTitle(t){
    var div = document.getElementById('display-area')
